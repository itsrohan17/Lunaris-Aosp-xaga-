...
#### ➕ Added
- Added storage save option for clipboard-only screenshots.
- Exposed supported blur algorithms for user selection.
- Added back system animation styles.
- Introduced initial **Dynamic Island** implementation (basic).
- Added option to hide the status bar while capturing screenshots.
- Added squish animation when tapping the Dynamic Island chip.
- Dynamic Island media popup seekbar now follows the system seekbar.
- Added one-time hotspot data limit.
- Updated Atmos wallpapers.
- Made minimum and maximum fling velocity user-configurable.

#### ⚡ Improvements
- Performed additional base cleanup.
- Updated Optimized Routines to **v26.07**.
- Reworked Dynamic Island popup animation.
- Switched Launcher3 back to clamp blur.

#### 🐞 Fixes
- Fixed Smart 5G service.
- Fixed random reboot caused by CacheAppThread NPE.
- Fixed notification count not being dismissed.
- Prevented ActivityThread NPE caused by null configuration.
- Removed broken app icon from Status Bar Lyrics to restore empty spacing.
- Fixed empty space when Dynamic Bar status bar media is disabled.
- Decoupled Dynamic Bar keyguard music pill from the status bar media toggle.
